...
# ╔═╡ 6ae899cd-213e-4150-a36a-0427864c93c4
atest="""1abc2
pqr3stu8vwx
a1b2c3d4e5f
treb7uchet"""

# ╔═╡ df66d471-5c19-4b48-bbf6-639f2ed02ef2
a="1abc"

# ╔═╡ 95e05312-ee93-4428-970e-52ee77db30cc
replace(a,r"[a-z]"=>"")

# ╔═╡ 58a76adc-2e1e-4054-8f2f-3c38204c4161
ainput="""51591twosix4dhsxvgghxq
425nine
llvmhjtr8nbbhrfone
...
function partA(d)
	l=d |> IOBuffer |> readlines

	sum=0
	for s in l
		n=replace(s,r"[a-z]"=>"")
		fl=n[begin]*n[end]
		sum=sum+parse(Int,fl)
	end
	sum
end
...
# ╔═╡ 44a73930-ef22-459b-9e48-aa878f6decad
partA(atest)==142

# ╔═╡ 3894efe8-027a-4fd8-bb14-29be0fddb1d1
partA(ainput) # Part A answer

# ╔═╡ 303c43ab-e1c4-4920-99b6-e2587e9e54c0
md"# Part B - Interpret 'one' 'two'"

# ╔═╡ 16fe3eaa-d93f-40ff-b072-5da9453364c8
btest="""
two1nine
eightwothree
abcone2threexyz
...
function partB(d)
	...
	sum=0
	for s in l
		n=replace(s, "one"=>"on1e", "two"=>"tw2o", "three"=>"thr3ee", "four"=>"fou4r",
			"five"=>"fi5ve", "six"=>"si6x", "seven"=>"sev7en","eight"=>"eig8ht", "nine"=>"ni9ne")
		n=replace(n, "one"=>"on1e", "two"=>"tw2o", "three"=>"thr3ee", "four"=>"fou4r",
			"five"=>"fi5ve", "six"=>"si6x", "seven"=>"sev7en","eight"=>"eig8ht", "nine"=>"ni9ne")
...
Part B quite frustrating with the GPT catching edge cases! 
And then I hadn't realised 'replace' does not work on the substitution text, but then thought that a hack would be to just to run the replacement twice, so a bit of copy and paste and off we go.
"""
