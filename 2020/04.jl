...
# ╔═╡ 379f5c9a-3623-11eb-06ac-8f599f21dcba
md"### Advent of Code 2020 - Day 04"

# ╔═╡ 5673fb76-3623-11eb-0e67-3de2dcf10825
md"### Part A - Validating passports"

# ╔═╡ 5d8e43d2-3623-11eb-0e77-9b08c2ba75f6
test="""
...
# ╔═╡ 6e4067c6-3623-11eb-3f44-abf2dbd49674
input="""
...
# ╔═╡ d8fad150-3623-11eb-0183-4d12cd2153b7
d=replace.(split(input,r"\n\n"), "\n" => " ")

# ╔═╡ 3d2fdcc8-3625-11eb-23b8-e1327553b858
split(d[1]) # seeing how the data will look to the function

# ╔═╡ fe520684-3624-11eb-0d49-5723984f0de0
function validate(passport)
	pdict=Dict()
	push!(pdict, "cid"=>"dummy") # always validate north pole
	for field in split(passport)
		key,data=split(field,":")
		push!(pdict, key => data)
	end
	
	pdict.count == 8 # return true if 8 fields defined
end

# ╔═╡ 8f299122-3625-11eb-3db8-bf3d50a2dd0f
pdict=validate(d[1]) # while programming, to see what partial computation the function returns

# ╔═╡ edb5e0e8-3624-11eb-0d78-1f4c74f5fbd2
sum(validate.(d))

# ╔═╡ 9e8475e8-3626-11eb-3186-4bf15a808d98
md" ### Part B: More detailed validation"

# ╔═╡ bb7cbe5e-3626-11eb-28e3-ed407c10a065
testinvalid="""
eyr:1972 cid:100
...
# ╔═╡ ceb6e1f2-3626-11eb-28d0-81da02b1a28c
testvalid="""
...
# ╔═╡ 1f40a7f8-3641-11eb-39ff-976c131b3aea
md"""
...
# ╔═╡ d2963f08-3640-11eb-3f5b-ef7b738174f4
function carefulvalidate(passport)
	p=Dict()
	push!(p, "cid"=>"dummy") # always validate north pole
	for field in split(passport)
		key,data=split(field,":")
		push!(p, key => data)
	end
	
	if p.count != 8 # stops failures on missing key lookups below
		return false
	end
	
	
	length(p["byr"])==4 && length(p["iyr"])==4 && length(p["eyr"])==4 &&
	length(p["hcl"]) == 7 && p["hcl"][1]=='#' && 
	length(p["pid"]) == 9 && 
	1920 <= parse(Int,p["byr"]) <= 2002 &&
	2010 <= parse(Int,p["iyr"]) <= 2020 &&
	2020 <= parse(Int,p["eyr"]) <= 2030 &&
	begin # I was really pleased with this idea, but it was too clever - I introduced bugs by using 'return' thinking this just went to end of begin/end block (no idea why!)
		hgt=p["hgt"]
		if hgt[end-1:end]=="cm"
			150 <= parse(Int,hgt[1:end-2]) <= 193
		elseif hgt[end-1:end]=="in"
			59 <= parse(Int,hgt[1:end-2]) <= 76
		else
			false
		end
	end &&
	0<= parse(Int,p["hcl"][2:end], base=16) <= 0xffffff &&
	match(r"amb|blu|brn|gry|grn|hzl|oth",p["ecl"])!=nothing && length(p["ecl"])==3 &&
	0<= parse(Int,p["pid"]) < 999999999 &&
	p.count == 8 # return true if 8 fields defined
end

# ╔═╡ 57e443da-3641-11eb-0a97-37c92e96a48c
p=carefulvalidate(d[1]) # see return on single datum; useful when partial function written

# ╔═╡ de97e6da-3640-11eb-04cc-37b3bf250882
carefulvalidate.(replace.(split(testinvalid,r"\n\n"), "\n" => " "))

# ╔═╡ fa13dfc2-3640-11eb-129f-750c5d9bbd30
carefulvalidate.(replace.(split(testvalid,r"\n\n"), "\n" => " "))

# ╔═╡ 4a8b9fd8-3643-11eb-20f4-33e08e2fe7b7
count(carefulvalidate.(d)) # stuck at 226; OK - 224 with a correcte begin/end block!

# ╔═╡ 80674a8c-366c-11eb-2ae7-d947fc78a239
length(d) # total passports

# ╔═╡ 692aabdc-3646-11eb-33a0-5b1220f38b1a
filter(d->carefulvalidate(d),d) # spit out which records match the validator, to see what I'm missing
# 226

# ╔═╡ 1658190e-3647-11eb-292b-e99ad03a43da
match(r"amb|blu|brn|gry|grn|hzl|oth","othhzl")!=nothing

# ╔═╡ 225dac52-3670-11eb-2807-e909c306ee5d
md"""
### Retrospective

I really enjoyed Part A. The data was super quick to read in, and it was elegant using a Dict to save the data for later. 

My heart sank when I read Part B. Data validation is boring and finnickity, and there's always edge cases. It's exactly the kind of programming I don't like, and as a scientific programmer, I don't generally have to do much of. I was intending to just not attempt it, but then thought I might be able to chain together logic like I ended up implementing. So it wasn't too long with lots of if/else functions.

I was kinda pleased with the little automtic testing bit for the 4 valid and 4 invalid cases, as I then added tests to the validation function. 

The begin/end block for height and switching on cm/in was my undoing. Too clever by half. I introduced a bug by 'return'ing the value, which of course returned the whole function at that point, without doing any further checks. 
"""
