...
md"### Advent of Code 2024 - Day 03"

# ╔═╡ 6a1dab04-f860-4a26-8c59-3f0298bf4ddb
md"# Part A - Mull It Over"

# ╔═╡ 1a545221-5aea-4cdf-bd78-073cdc2cfd2e
function partA_calcline(l)
	rx = r"mul\(([0-9]+)\,([0-9]+)\)"
	ms=eachmatch(rx,l)
	sum([parse(Int,m.captures[1])*parse(Int,m.captures[2]) for m in ms])
#	return m
end
...
function partA(d)
	...
	sum([partA_calcline(l) for l in ls]) # old per-line structure; see warning in notes!
end

# ╔═╡ ad4c09d4-4fde-417f-bd90-b34e7244aebe


# ╔═╡ 303c43ab-e1c4-4920-99b6-e2587e9e54c0
md"# Part B - (Do Don't)"

# ╔═╡ 14019601-cdf4-4a19-81e9-15cc2cde6b3d
testB="""xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))"""

# ╔═╡ 1dc02cf7-77cc-46c5-b9d5-8c7c647b3140
function partB_calcline(l)
# delete the sections between don't() and do() functions - these are ignored
# note non-greedy '.*?' rather than greedy '.*' matchall chars
	rx = r"don't\(\).*?do\(\)"
	l=replace(l,rx=>"")
	l=replace(l,r"don't\(\).*"=>"") # and any last non-terminated don'ts
...
function partB(d)
	...
	sum([partB_calcline(l) for l in ls])
end

# ╔═╡ c36f4493-a5ad-4eef-8837-b8adbfd10d2a
partB(testB)

# ╔═╡ 8eba9c98-eb01-492b-8b16-ef48bd124f51
partB(" do() YY don't() NNN do() don't() NNdo()YYY don't() NN don't() NN do() Y don't()")

# ╔═╡ 050479b3-e2fb-43ea-bfea-cffa49d3c21c
partB(testB)==48

# ╔═╡ ea382994-b79f-4786-8839-d8af62f2d084
# 88125394
...
Incredibly frustrating on partB, as my two-minute solution was CORRECT, but I hadn't realised that the input was meant to be considered as one line / memory chunk! So I was passing per line (there were about five), and therefore some of the "don't()" should have passed onto the next section of the memory. 

Otherwise, like the old joke: Regexp are useful for 99 problems, but then you have 100 problems.

It was nice to be reminded of the Julia regexp and string mangling syntax. 
"""
